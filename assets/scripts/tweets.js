...
(function(root, factory) {
  ...
  var twitterFetcher = {
    fetch: function(config) {
      ...
      config.maxTweets = config.maxTweets || 20;

      config.enableLinks = config.enableLinks || false;
      config.showAvatar = config.showAvatar || true;
      config.showUser = config.showUser || true;
      config.showTime = config.showTime || true;
      config.showRetweet = config.showRetweet || false;
      config.showInteraction = config.showInteraction || false;
      config.showImages = config.showImages || false;
      config.useEmoji = config.useEmoji || false;
      config.linksInNewWindow = config.linksInNewWindow || true;
      config.showPermalinks = config.showPermalinks || true;
      config.dataOnly = config.dataOnly || false;
      config.dateFunction = config.dateFunction || 'default';
      config.customCallback = config.customCallback || null;
...
function dateFormatter(date) {
  let tweetDay = date.getDate() < 10 ? '0' + date.getDate() : date.getDate();
  let tweetMonth = (date.getMonth() + 1) < 10 ? '0' + (date.getMonth() + 1) : (date.getMonth() + 1);
  return tweetDay + "." + tweetMonth + "." + date.getFullYear().toString().slice(2);
